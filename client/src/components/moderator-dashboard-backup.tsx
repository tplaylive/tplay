...
export default function AdminDashboard({ userRole, username }: AdminDashboardProps) {
  ...
  const updateModeratorMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setEditingModerator(null);
      moderatorForm.reset();
      toast({ title: 'Success', description: 'Moderator updated successfully' });
    },
    onError: (error) => {
              ...
              <DialogContent className="bg-slate-800 border-slate-700 text-white max-w-lg">
                ...
                <Form {...channelForm}>
                  <form onSubmit={channelForm.handleSubmit(handleChannelSubmit)} className="space-y-4">
                      ...
                      render={({ field }) => (
                        <FormItem className="flex flex-row items-center justify-between rounded-lg border border-slate-600 p-3 shadow-sm">
                          <div className="space-y-0.5">
                            <FormLabel className="text-base">DRM Protected Content</FormLabel>
                            <div className="text-sm text-muted-foreground">
                              Enable for encrypted channels with clearkey DRM
                            </div>
                          </div>
                          <FormControl>
                            <Switch
                    ...
                    {/* DRM Fields - Only show when DRM is enabled */}
                    {channelForm.watch('isDrmProtected') && (
                      <>
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel>Manifest URL (.mpd)</FormLabel>
                              <FormControl>
                                <Input 
                                  {...field} 
                                  className="bg-slate-700 border-slate-600" 
        ...
        {userRole === 'admin' && (
          <TabsContent value="moderators" className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <Dialog open={isModeratorDialogOpen} onOpenChange={() => {
                setIsModeratorDialogOpen(false);
                setEditingModerator(null);
                moderatorForm.reset();
              }}>
                <DialogTrigger asChild>
                  <Button 
                    onClick={() => {
                      setEditingModerator(null);
                      moderatorForm.reset({ username: '', password: '', role: 'moderator' });
                      setIsModeratorDialogOpen(true);
                    }}
